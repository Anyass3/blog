...
export const handle: Handle = async ({ event, resolve }) => {
	...
	if (event.request.method != 'POST' && MM_PUBLIC_KEY && !locals) {
		...
		event.locals.signedToken = E.sign(event.locals.token, event.locals.signKeyPair.signSecretKeyHex)
		event.locals.encryptedToken = await E.encrypt(event.locals.token, event.locals.keyPair.publicKeyHex);
		const data = `${event.locals.signKeyPair.signPublicKeyHex}::${event.locals.keyPair.publicKeyHex}::${event.locals.signedToken}`;
		event.locals.encryptedData = await E.encrypt(data, Buffer.from(MM_PUBLIC_KEY, 'base64').toString('hex'));
		event.locals.expires = Date.now() + 24 * 3.6e+6; // expires in 24 hours
	} else {
