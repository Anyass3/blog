...
export const POST = async ({ cookies, request, locals }) => {
	...
	if (action == 'login' && MM_PUBLIC_KEY) {
		...
		locals.signedToken = E.sign(locals.token, locals.signKeyPair.signSecretKeyHex);
		locals.encryptedToken = await E.encrypt(locals.token, locals.keyPair.publicKeyHex);
